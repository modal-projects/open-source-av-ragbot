...
import modal
from .kokoro_tts import FORMAT, kokoro_tts_dict, recvbuffer, sendbuffer
...
class KokoroTTSService(TTSService):
    ...
    async def start(self, frame: StartFrame):
        ...
        await kokoro_tts_dict.put.aio(self.dict_id, True)
        
        print("Kokoro TTS server ready")
        kokoro_cls = modal.Cls.from_name("kokoro-tts", "KokoroTTS")
        kokoro_cls().run.spawn(self.dict_id)
        # await asyncio.sleep(10)
        while not await kokoro_tts_dict.get.aio("server_ready"):
            await asyncio.sleep(0.1)
        # await asyncio.sleep(1.0)
        print("Kokoro TTS server ready")
        print("Getting host and port from Kokoro TTS")
        host = await kokoro_tts_dict.get.aio("host")
        port = await kokoro_tts_dict.get.aio("port")
