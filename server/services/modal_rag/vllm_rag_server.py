...
class VLLMRAGServer:
    ...
    def setup(self):
        ...
        if not os.path.exists(self.model_path):
            self.download_models(LLM_MODEL, MODELS_DIR)
        ...
        #     self.retriever = vector_index.as_retriever(similarity_top_k=2)
    ...
    def download_models(self, model_ids, download_dir):
        from huggingface_hub import snapshot_download
        if not isinstance(model_ids, list):
            model_ids = [model_ids]
        print("Downloading models...")
        for repo_id in model_ids:
            snapshot_download(repo_id=repo_id, local_dir=download_dir / repo_id)
            print(f"Model downloaded to {download_dir / repo_id}")
            ...
            async for generation in streaming_generator:
                if generation.outputs:
                    if first_token_time is None:
                        ...
                        vllm_start = first_token_time
                    else:
                        first_token_time = time.perf_counter()
                        print(f"⏱️ [Streaming] Next token took {first_token_time - vllm_start:.3f}s")
                        vllm_start = first_token_time
                    accumulated_text = generation.outputs[0].text
                    token_count += 1
                    
                    # if skip_send_count == 10:
                    #     skip_send_count = 0
